...
% secure_area = input('Activate secure area? (Type 1 or 0 for yes or no): '); %Activate secure area 
...
% BS_HPBW = find(BS_antenna_pattern(:,91) == max(BS_antenna_pattern(:,91))) - find(BS_antenna_pattern(:,91) == max(find(BS_antenna_pattern(:,91) <= max(BS_antenna_pattern(:,91)) - 3))) - 1;
...
BS = BS_deployer_hexagon_v2(0, radius(m), h_BS,area_of_interest, FS); %Obtains a matrix with information about BSs. Different rows indicate different BSs. Different columns indicate different BSs details: [1 2 3] -> (x,y,z) location, [4] -> operating mode (UL(1) or DL(2))
...
%         marked_for_use = [];
%         
%         if max((BS(BS_at_the_back_of_FS + 1,:))) == inf    %If auxiliary antenna is not used, removes it.
%             marked_for_remove = BS_at_the_back_of_FS + 1;
%         else
%             marked_for_use = BS_at_the_back_of_FS + 1;
%         end
%         
%         if max((BS(BS_at_the_back_of_FS + 2,:))) == inf    %If auxiliary antenna is not used, removes it.
%             marked_for_remove = [marked_for_remove BS_at_the_back_of_FS + 2];
%         else
%             marked_for_use = [marked_for_use BS_at_the_back_of_FS + 2];
%         end
%         
%         BS(marked_for_remove,:) = [];
%         
        BS_TXpower_aux = [];
...
%             users([BS_at_the_back_of_FS + 1 BS_at_the_back_of_FS + 2], :, :) = inf;
    ...
    hold on
%     plot(x,y, char(style(m)), 'LineWidth',2)
...
xlabel('Interference level (dBm)')
ylabel('Percentage of timeslots under a certain interference level' )
...
% legend(['R = ', num2str(radius(1))], ['R = ', num2str(radius(2))], ['R = ', num2str(radius(3))])
xlabel('BS transmission power (dBm)')
ylabel('Percentage of BSs under a certain transmission power' )
...
% legend(['R = ', num2str(radius(1))], ['R = ', num2str(radius(2))], ['R = ', num2str(radius(3))])
xlabel('Interference level (dBm)')
ylabel('Percentage of BSs under a certain interference level' )
legend(['R = ', num2str(radius(1)), ' -- No SA'], ['R = ', num2str(radius(1)), ' -- SA'], ['R = ', num2str(radius(2)), ' -- No SA'], ['R = ', num2str(radius(2)), ' -- SA'], ['R = ', num2str(radius(3)), ' -- No SA'], ['R = ', num2str(radius(3)), ' -- SA'], 'I_{thr}')
